...
from logistic_sgd import LogisticRegression, load_apk_data
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=800,
             dataset='bkclass.pkl.gz', batch_size=20, n_hidden=50):
    """
...
def load_vision_data(dataset):
    ...
    #row_sums = train_x.sum(axis=0)
    #train_x = train_x / row_sums[numpy.newaxis,:]
    # dispatch xs and ys to train,test and validation set
...
def test_mlp_fiidd(learning_rate=0.001, L1_reg=0.00, L2_reg=0.001, n_epochs=1000,
             dataset=sys.argv[1], batch_size=20, n_hidden=int(sys.argv[2])):
    """
    Demonstrate stochastic gradient descent optimization for a multilayer
    perceptron
   ...
   """
    datasets = load_apk_data(dataset)
    train_set_x, train_set_y = datasets[0]
    valid_set_x, valid_set_y = datasets[1]
    test_set_x, test_set_y = datasets[2]
    ...
    # construct the MLP class
    classifier = MLP(rng=rng, input=x, n_in=input_dimension,
    ...
    pp_error = theano.function(inputs=[index],
            outputs=classifier.pp_errors(y,0.90,1),
            givens={
                x: test_set_x[index * batch_size:(index + 1) * batch_size],
                y: test_set_y[index * batch_size:(index + 1) * batch_size]})
    ...
    while (epoch < n_epochs) and (True or not done_looping) and (not too_fit):
        ...
        if predn>0:
            ...
            pp=0.0
        print 'e %i, verr %2.1f %% , merr %2.1f %% , pp %i, n %i , pp %2.1f %% \r' % (epoch, this_validation_loss * 100., this_model_loss*100., ppn,predn, pp*100.),
        sys.stdout.flush()
        #print '[learning] epoch %i , model acc %2.2f %%>> '%(epoch,ppn),'completed in %.2f (sec) <<\r'%(time.time()-start_time),
        #sys.stdout.flush()
        
        if this_model_loss<0.01:
            too_fit=True
